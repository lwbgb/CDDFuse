...
class BiasFree_LayerNorm_Optimized(nn.Module):
    def __init__(self, normalized_shape):
        super(BiasFree_LayerNorm_Optimized, self).__init__()
        if isinstance(normalized_shape, numbers.Integral):
            normalized_shape = (normalized_shape,)
        normalized_shape = torch.Size(normalized_shape)
    ...
    def forward(self, x):
        ...
        # 🚀 内存优化小技巧：先计算出尺度因子 (只有 Channel 维度大小)，再与巨大的 x 相乘
        # 这比原来的写法减少了一个等同于 x 体积的除法中间变量
        scale_factor = self.weight / torch.sqrt(sigma + 1e-5)
...
class WithBias_LayerNorm_Optimized(nn.Module):
    ...
    def forward(self, x):
        # 🚀 核心优化：使用 PyTorch 底层 C++ 算子替代手动拼凑
        # 它会在底层完成 (x-mu)/sqrt(var+eps)*weight + bias，不仅零额外显存，而且速度快几倍
...
class LayerNorm(nn.Module):
    def __init__(self, dim, LayerNorm_type):
        ...
        if LayerNorm_type == 'BiasFree':
            ...
            self.body = BiasFree_LayerNorm_Optimized(dim)
        else:
