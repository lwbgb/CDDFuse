    ...
    for epoch in range(opt.start_epoch, opt.n_epochs + 1):
        ...
        for batch_idx, (data_VIS, data_IR) in enumerate(pbar):
            ...
            if current_phase == 1:  # Phase I
                feature_VIS_Base, feature_VIS_Detail, _ = DIDF_Encoder(data_VIS)
                feature_IR_Base, feature_IR_Detail, _ = DIDF_Encoder(data_IR)
                data_VIS_hat, _ = DIDF_Decoder(data_VIS, feature_VIS_Base, feature_VIS_Detail)
                data_IR_hat, _ = DIDF_Decoder(data_IR, feature_IR_Base, feature_IR_Detail)

                cc_loss_B = cc(feature_VIS_Base, feature_IR_Base)
                cc_loss_D = cc(feature_VIS_Detail, feature_IR_Detail)
        ...
        for optimizer in optimizers.values():
            ...
            optimizer.param_groups[0]["lr"] = max(lr, opt.min_lr)
